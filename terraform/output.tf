...
resource "local_sensitive_file" "env_file" {
  content = join("\n", [
    ...
    "userPoolID=${aws_cognito_user_pool.lifted.id}",
    "AWS_REGION=${var.aws_region}",
    "API_KEY=${random_uuid.test.result}",
    "PROFILE_URL=${aws_cloudfront_distribution.liftedprofiles_distribution.domain_name}",
    local.file_contents
  ])
